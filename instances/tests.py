...
class MakeLicenceKeyCommandTests(TestCase):
    def test_it_prints_a_usable_pair(self):
        ...
        verify. So the command is the only supported way to make one, and this
        proves what it prints round-trips."""
        out = StringIO()
        call_command('make_licence_key', stdout=out)
        ...
        private_b64 = private_line.split('=', 1)[1]
        public_line = [
            line for line in printed.splitlines()
            if line.startswith('LICENCE_PUBLIC_KEY=')
        ][0]
        public_b64 = public_line.split('=', 1)[1]
        ...
        signature = key.sign(b'a licence document')
        public.verify(signature, b'a licence document')
